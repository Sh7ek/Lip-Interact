...
class LipNet(object):
    ...
    def build(self):
        if K.image_data_format() == 'channels_first':
            ...
            input_shape = (self.frames_n, self.img_h, self.img_w, self.img_c)  # our case

        self.input_data = Input(name='input', shape=input_shape, dtype='float32')

        # 70 x 80 x 100 x 3
        self.zero1 = ZeroPadding3D(padding=(1, 3, 3), name='zero1')(self.input_data)
        self.conv1 = Conv3D(32, (3, 7, 7), strides=(1, 3, 3), kernel_initializer='he_normal', name='conv_1')(self.zero1)
        # 70, 27, 34, 32
        ...
        self.gru_1 = Bidirectional(GRU(96, return_sequences=True, kernel_initializer='Orthogonal', name='gru1'),merge_mode='concat')(self.resh1)
        # 70 x (128 x 2) = 70 x 256
        self.gru_2 = Bidirectional(GRU(96, kernel_initializer='Orthogonal', name='gru2'), merge_mode='concat')(self.gru_1)
        # 192

        # transforms RNN output to classification:
        self.prediction = Dense(units=self.output_n, activation='softmax', kernel_initializer='he_normal', name='predict')(self.gru_2)
        # 10
...
if __name__ == "__main__":
    # create model
    lipnet = LipNet(frames_n=70, img_h=80, img_w=100, img_c=3, output_n=10)
    lipnet.summary()
